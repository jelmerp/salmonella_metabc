...
After quality control of FASTQ files with `fastqc` (version 0.11.8, Andrews 2010)
and `multiqc` (version 1.11, Ewels et al. 2016),
primers were removed from each read using `cutadapt` (version 3.4, Martin 2011)
and read pairs for which a primer was not detected either for the forward or reverse
read were removed.

The R/Bioconductor package `DADA2` (version 1.16, Callahan et al. 2016) was used
...
using the Silva (Quast et al. 2013) database
...
(version 1.14.0, Davis et al. 2018) using 4 negative control samples,
